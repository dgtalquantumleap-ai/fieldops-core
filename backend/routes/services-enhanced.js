    ...
    try {
        ...
        if (include_analytics === 'true') {
            for (const service of services) {
                let trendParams = [service.id];
                let trendFilters = '';
                let pidx = 2;
                if (date_from) { trendFilters += ` AND j2.job_date >= $${pidx++}`; trendParams.push(date_from); }
                if (date_to)   { trendFilters += ` AND j2.job_date <= $${pidx++}`; trendParams.push(date_to); }

                const [monthlyTrends, staffPerformance, satisfaction] = await Promise.all([
                    db.query(`
                        SELECT
                            TO_CHAR(j2.job_date::DATE, 'YYYY-MM') as month,
                            COUNT(*) as jobs,
                            SUM(COALESCE(j2.final_price, s2.price, 0)) as revenue
                        FROM jobs j2
                        LEFT JOIN services s2 ON j2.service_id = s2.id
                        WHERE j2.service_id = $1 AND j2.status = 'completed'
                        ${trendFilters}
                        GROUP BY TO_CHAR(j2.job_date::DATE, 'YYYY-MM')
                        ORDER BY month DESC
                        LIMIT 12
                    `, trendParams),
                    db.query(`
                        ...
                        WHERE j.service_id = $1 AND j.status = 'completed'
                    ...
                    `, [service.id]).catch(() => ({ rows: [{ avg_rating: 0, total_ratings: 0 }] })),
                ]);

                service.analytics = {
                    monthlyTrends: monthlyTrends.rows,
                    staffPerformance: staffPerformance.rows,
                    satisfaction: satisfaction.rows[0] || { avg_rating: 0, total_ratings: 0 },
...
router.post('/enhanced', requireAdmin, async (req, res) => {
    try {
        ...
        if (!name || !description || !price) {
            return res.status(400).json({ success: false, error: 'Name, description, and base price are required' });
        }
        ...
        const result = await db.query(`
            ...
            RETURNING id
        `, [
            name.trim(), description.trim(), parseFloat(price),
            parseInt(duration) || 2, category || 'General',
        ...
        if (io) emitRealTimeUpdate(io, 'service-created', { service }, 'admin');

        res.status(201).json({ success: true, message: 'Enhanced service created successfully', data: service });
...
router.put('/enhanced/:id', requireAdmin, async (req, res) => {
    try {
        ...
        const existingService = (await db.query('SELECT * FROM services WHERE id = $1', [serviceId])).rows[0];
        if (!existingService) {
        ...
        await db.query(`
            UPDATE services SET
                name=$1, description=$2, price=$3, duration=$4, category=$5,
                difficulty_level=$6, required_skills=$7, equipment_needed=$8,
                pricing_model=$9, min_price=$10, max_price=$11, price_per_hour=$12,
                tags=$13, is_active=$14, updated_at=NOW()
            WHERE id=$15
        `, [
        ...
        const io = req.app.get('io');
        if (io) emitRealTimeUpdate(io, 'service-updated', { service }, 'admin');
...
router.get('/analytics', requireAuth, async (req, res) => {
    try {
        ...
        const serviceFilter = safeServiceId ? 'AND j.service_id = $1' : '';
        const serviceParams = safeServiceId ? [safeServiceId] : [];

        const [revenueTrends, serviceComparison, popularServices, staffExpertise] = await Promise.all([
            db.query(`
                SELECT
                    TO_CHAR(j.job_date::DATE,'YYYY-MM-DD') as date,
                ...
                ${serviceFilter}
                GROUP BY TO_CHAR(j.job_date::DATE,'YYYY-MM-DD'), s.id, s.name
                ORDER BY date DESC
            `, serviceParams),
            db.query(`
                SELECT
                ...
                LEFT JOIN jobs j ON s.id = j.service_id
                WHERE s.is_active = 1
                AND (j.job_date >= (NOW() - INTERVAL '${safePeriod} days')::DATE::TEXT OR j.id IS NULL)
            ...
            db.query(`
                ...
                WHERE j.status = 'completed'
                AND j.job_date >= (NOW() - INTERVAL '${safePeriod} days')::DATE::TEXT
                GROUP BY st.id, st.name, s.id, s.name
                ORDER BY jobs_completed DESC
            `),
        ]);
        ...
        res.json({
            ...
            data: {
                ...
                serviceComparison: serviceComparison.rows,
                popularServices: popularServices.rows,
                staffExpertise: staffExpertise.rows,
                period: `${period} days`
            }
        });
...
router.post('/:id/duplicate', requireAdmin, async (req, res) => {
    try {
        ...
        const originalService = (await db.query('SELECT * FROM services WHERE id = $1', [serviceId])).rows[0];
        if (!originalService) {
            return res.status(404).json({ success: false, error: 'Service not found' });
        }
        ...
        logActivity(req.user?.id, req.user?.name, 'duplicated', 'service', newId, originalService.name);

        const duplicatedService = (await db.query('SELECT * FROM services WHERE id = $1', [newId])).rows[0];

        res.status(201).json({ success: true, message: 'Service duplicated successfully', data: duplicatedService });
